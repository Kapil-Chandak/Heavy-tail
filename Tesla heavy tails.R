...
a<-read.csv("C:/Users/chand/Downloads/AAPL (2).csv")
a
ret<-rep(0,length(a$Close)-1)
lret<-rep(0,length(a$Close)-1)
for(i in 1:length(a$Close)-1){ret[i]<-((a$Close[i+1]-a$Close[i])/a$Close[i])}
for(i in 1:length(a$Close)-1){lret[i]<-(log(a$Close[i+1])-log(a$Close[i]))/log(a$Close[i])}
plot(a$Close,main = "Closing price of Apple stocks over a year",ylab = "Closing price",xlab = "Day")
...
a<-read.csv("C:/Users/chand/Downloads/INDEX.csv")
a
ret<-rep(0,length(a$Close)-1)
lret<-rep(0,length(a$Close)-1)
for(i in 1:length(a$Close)-1){ret[i]<-((a$Close[i+1]-a$Close[i])/a$Close[i])}
for(i in 1:length(a$Close)-1){lret[i]<-(log(a$Close[i+1])-log(a$Close[i]))/log(a$Close[i])}
plot(a$Close,main = "Closing price of S&P 500 over a year",ylab = "Closing price",xlab = "Day")
